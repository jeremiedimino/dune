...
module Dir = struct
  ...
  let map_reduce (type a) (module M : Monoid.S with type t = a) t ~traverse ~f =
    let rec loop t =
      let must_traverse = Sub_dirs.Status.Map.find traverse t.status in
      match must_traverse with
      | false -> Memo.Build.return M.empty
      | true ->
        let+ here = f t
        and+ in_sub_dirs =
          Memo.Build.parallel_map (String.Map.values t.contents.sub_dirs)
            ~f:(fun s -> sub_dir_as_t s >>= loop)
        in
        List.fold_left in_sub_dirs ~init:here ~f:M.combine
    in
    loop t
