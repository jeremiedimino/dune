...
module External : sig
  ...
  let t =
    Sexp.Of_sexp.(map_result string) ~f:(fun t ->
      if Filename.is_relative t then
...
module Local : sig
  ...
  let t =
    Sexp.Of_sexp.(map_with_loc string) ~f:(fun ~loc s ->
      of_string s ~error_loc:loc)
...
let t =
  ...
  string_or_list (fun ~loc:_ s -> of_string s)
    (sum
