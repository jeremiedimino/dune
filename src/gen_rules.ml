...
module Gen(P : Install_rules.Params) = struct
  ...
  module Dir_contents : sig
    type t =
      { (* Set of "text" files (.ml, .c, ...). This is the list of
           source files + user generated ones. *)
        text_files : String.Set.t
      ; (* Has at least one [library] or [executables] stanza *)
        has_buildable : bool
      ; modules : Module.t String.Map.t Lazy.t
    ...
    let scan_stanzas ~dir =
      match Path.Map.find stanzas_per_dir dir with
      | None ->
        { text_files = String.Set.empty
        ; has_buildable = false
        ; modules = lazy String.Map.empty
        ; mlds = lazy String.Map.empty
        }
      | Some { stanzas; src_dir; scope; _ } ->
        ...
        let has_buildable =
          List.exists stanzas ~f:(function
            | Library _ | Executables _ | Tests _ -> true
            | _ -> false)
        in
        (text_files, has_buildable)
